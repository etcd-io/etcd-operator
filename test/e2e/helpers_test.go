...
func waitForClusterHealthyStatus(t *testing.T, c *envconf.Config, name string, expectedMembers int) ecv1alpha1.EtcdCluster {
	t.Helper()
	...
	err := wait.For(func(ctx context.Context) (bool, error) {
		...
		if !conditionEquals(cluster.Status.Conditions, status.ConditionAvailable, metav1.ConditionTrue, status.ReasonClusterReady) {
			return false, nil
		}
		if !conditionEquals(cluster.Status.Conditions, status.ConditionProgressing, metav1.ConditionFalse, status.ReasonReconcileSuccess) {
			return false, nil
		}
		if !conditionEquals(cluster.Status.Conditions, status.ConditionDegraded, metav1.ConditionFalse, status.ReasonClusterHealthy) {
			return false, nil
		}
...
func conditionEquals(conditions []metav1.Condition, condType string, condStatus metav1.ConditionStatus, condReason string) bool {
