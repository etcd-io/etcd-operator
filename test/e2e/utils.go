	...
	if err := wait.For(
		conditions.New(client.Resources()).ResourceMatch(object,
			func(object k8s.Object) bool {
				err := client.Resources().Get(ctx, resourceName, resourceNamespace, outObj)
				return err == nil
			}),
		options...,
	); err != nil {
		fmt.Println(outObj)
		return fmt.Errorf("unable to find %s in namespace %s: %s", resourceName, resourceNamespace, err)
	}
