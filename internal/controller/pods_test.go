...
func TestNextOrdinal(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:            "Missing pod-1 (Your original issue)",
			list:            []int{0, 2},
			expectedReplica: 3,
			want:            1,
		},
		...
		{
			name:            "Under-scaled cluster (Needs scale up)",
			list:            []int{0, 1},
			expectedReplica: 3,
			want:            2,
		},
		{
			name:            "Fully healthy cluster (No action needed)",
			list:            []int{0, 1, 2},
			expectedReplica: 3,
			want:            -1,
		},
		{
			name:            "User scenario with out-of-bounds numbers",
			list:            []int{0, 3, 9},
			expectedReplica: 5,
			want:            1,
		},
		...
		{
			name:            "Zero or negative expected replicas guard",
			list:            []int{0, 1, 2},
			expectedReplica: 0,
			want:            -1,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := nextPodOrdinal(tt.list, tt.expectedReplica)
